...
class MomoService {
    ...
    public function updateStock($all) {
        ...
        $productModels = $all ? Product::select('momo_id', 'momo_dt_code', 'stock')->groupBy('momo_id')->get() : Product::select('momo_id', 'momo_dt_code', 'stock')->where('updated_at', '>=', date('Y-m-d H:i:s', strtotime('-15 min')))->groupBy('momo_id')->get();
